...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    static let client = TwitterAPICaller(baseURL: URL(string: "https://api.twitter.com"), consumerKey: "SesZnN56GqOJpSzVh80n6bElB", consumerSecret: "7T20ShdtGC1Uldq2iV0kXSwWKaojQSfc4m1N3yZEoQo3sm6QVT")
    var loginSuccess: (() -> ())?
    ...
    func rewtweet(tweetID:Int, success: @escaping () -> (), failure: @escaping (Error) -> ()) {
        
        let url = "https://api.twitter.com/1.1/statuses/retweet/\(tweetID).json"
        
        TwitterAPICaller.client?.post(url, parameters: nil, progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
        })
